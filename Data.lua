...
-- The player's current map is also scanned at runtime, so events visible from
-- adjacent maps (e.g. Silvermoon City Prey POIs) still surface.
ns.zones = {
    2395,  -- Eversong Woods
    2405,  -- Voidstorm
    2413,  -- Harandar
...
-- Events that fire on a wall-clock cadence without a per-POI timer. Keyed by
-- short event name (everything before ": "). Non-:00/:30 phase events would
-- need an additive offset before the modulo here.
ns.fixedCadence = {
...
-- After a predicted firing passes, the entry stays in 'now!' state this long
-- before rolling forward to the next cadence mark.
ns.scheduleGracePeriod = 600  -- 10 min

-- ── Event toggle aliases ──────────────────────────────────────────────────────
-- Maps an observed event name (full or short) to a stable toggle key so POI
-- variants collapse onto one setting.
local EVENT_KEY = {
    ["Abundance"]                = "abundance",
